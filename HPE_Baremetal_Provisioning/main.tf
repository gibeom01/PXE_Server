  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
      ...
      ansible-playbook deploy_os.yml \
        -e "ilo_ip=${each.value.ilo_ip} ilo_user=${var.global_ilo_user} ilo_password=${var.global_ilo_password} mac_http_ip=${var.mac_http_ip} http_port=${each.value.http_port} iso_name=${each.value.iso_name}" \
