...
<unattend xmlns="urn:schemas-microsoft-com:unattend" xmlns:wcm="http://schemas.microsoft.com/WMIConfig/2002/State">
    ...
    <settings pass="oobeSystem">
        <component name="Microsoft-Windows-Shell-Setup" processorArchitecture="amd64" publicKeyToken="31bf3856ad364e35" language="neutral" versionScope="nonSxS">
            ...
            <UserAccounts><AdministratorPassword><Value>!Password2026</Value><PlainText>true</PlainText></AdministratorPassword></UserAccounts>
            <AutoLogon><Password><Value>!Password202</Value><PlainText>true</PlainText></Password><Enabled>true</Enabled><LogonCount>1</LogonCount><Username>Administrator</Username></AutoLogon>
            
            <FirstLogonCommands>
                <SynchronousCommand wcm:action="add">
                    <Order>1</Order>
                    <CommandLine>powershell -WindowStyle Hidden -Command "Start-Sleep -Seconds 10"</CommandLine>
                    <Description>Wait for Network Stack Initialization</Description>
                </SynchronousCommand>
                <SynchronousCommand wcm:action="add">
                    <Order>2</Order>
                    <CommandLine>powershell -WindowStyle Hidden -Command "Get-NetAdapter -Physical | Set-NetIPInterface -Dhcp Disabled"</CommandLine>
                    <Description>Disable DHCP</Description>
                </SynchronousCommand>
                <SynchronousCommand wcm:action="add">
                    <Order>3</Order>
                    <CommandLine>powershell -WindowStyle Hidden -Command "Get-NetAdapter -Physical | New-NetIPAddress -IPAddress $IpAddress -PrefixLength $PrefixLength -DefaultGateway $Gateway"</CommandLine>
                    <Description>Set Static IP</Description>
                </SynchronousCommand>
                <SynchronousCommand wcm:action="add">
                    <Order>4</Order>
                    <CommandLine>powershell -WindowStyle Hidden -Command "Get-NetAdapter -Physical | Set-DnsClientServerAddress -ServerAddresses $Dns"</CommandLine>
