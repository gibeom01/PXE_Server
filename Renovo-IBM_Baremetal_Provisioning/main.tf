...
resource "null_resource" "hardware_provisioning" {
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
      ...
      # 1. 펌웨어 및 RAID 구성 (Lenovo 같은 Redfish 지원 장비만 실행)
      if [ "${each.value.api_type}" = "redfish" ]; then
        echo "Redfish API를 통해 펌웨어 및 RAID 구성을 진행합니다."
        ansible-playbook update_firmware.yml \
          -e "bmc_ip=${each.value.bmc_ip} bmc_user=${var.global_bmc_user} bmc_password=${var.global_bmc_password}" >> ../logs/${each.key}_hw.log 2>&1
      ...
      ansible-playbook deploy_os.yml \
        -e "bmc_ip=${each.value.bmc_ip} bmc_user=${var.global_bmc_user} bmc_password=${var.global_bmc_password} mac_http_ip=${var.mac_http_ip} http_port=${each.value.http_port} iso_name=${each.value.iso_name} api_type=${each.value.api_type}" >> ../logs/${each.key}_os_deploy.log 2>&1
