...
- Renovo 서버(XCC 초기 IP: 192.168.0.120) 기준 Mac에 래선 젠더 꽂고 랜 케이블 -> 서버 뒷면 XCC 전용 포트(스패너 모양) 연결.
...
- 1단계: 자동화 환경 준비 (Mac Air)                    -> Renovo XCC 제어할 Ansible 모듈과 Redfish API 통신 필요할 Python 라이브러리 설치.
- 2단계: 하드웨어 펌웨어 최적화 (XCC)                  -> 서버의 XCC 관리 포트(IP: 10.10.10.120)로 접속해 펌웨어 설치 파일 업로드.
- 3단계: 물리 디스크 RAID 및 스토리지 구성 (XCC)        -> 디스크 설정 밀고, 전면 베이 꽂힌 1.2TB 디스크 2개 묶어 RAID 1(미러링) 볼륨 생성.
- 4단계: Windows OS 무인 설치 (XCC & 가상 미디어)     -> HTTP 서버 통해, 무인 설치 응답 파일 포함된 Windows Server ISO XCC의 가상 CD-ROM 마운트.
...
- 해당 폴더 ~/Desktop/Lenovo_XCC_SR630_Windows 위치에 옮겨 진행.
...
- cd ~/Desktop/Lenovo_XCC_SR630_Windows/ISO             -> MAC 터미널 복사 후 붙여넣기 진행. (파일이 생성된 경로 이동.)
- python3 -m http.server 8000                       -> MAC 터미널 복사 후 붙여넣기 진행. (XCC 접속할 수 있도록 웹서버 오픈.)
